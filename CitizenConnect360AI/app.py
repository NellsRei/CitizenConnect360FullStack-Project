...
def get_user(userid):
    ...
    getUser = f"SELECT * FROM users WHERE userid=?"
    

    print(userid) 

    try:
        data = query_db(getUser, (userid,))    
        return jsonify(data), 200
...
def register_user():
    ...
    # Validate input data
    if not all([id, username, email, password, role]):
    ...
    try:
        conn = pyodbc.connect(connectionString)
        cursor = conn.cursor()
        cursor.execute(registerUser, (id, username, email,password,role))
        conn.commit()
        conn.close()
